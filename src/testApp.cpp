...
void testApp::startKinect() {
	//
}

//--------------------------------------------------------------
void testApp::updateIdleMode() {
	/*updateDepthStream();*/
	checkActiveBlocks();
}

void testApp::updateRecoMode() {
	/*updateDepthStream();*/
	checkActiveBlocks();
...
void testApp::switchMode(int mode) {
	...
	switch(mode) {
		case 'idle':
			isIdleMode = true;
			soundIdle.play();
			// this is specific for the current idle sound
			soundIdle.setPosition(0.1f);
			break;
		case 'reco':
			isRecoMode = true;
			soundReco.play();
...
void testApp::drawGameOverZone() {
	...
	// turn it back off, if you don't need it
	ofDisableAlphaBlending();
}
