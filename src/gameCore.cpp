...
void gameCore::startKinect() {
	
	/* all nodes created by code (no xml file) */

	streamContext.setup();
	streamDepth.setup(&streamContext);
	
	streamContext.toggleRegisterViewport();
	streamContext.toggleMirror();
...
void gameCore::update() {
	...
	if (isIdleMode) updateIdleMode();
	else if (isRecoMode) updateRecoMode();
	else if (isGameMode) updateGameMode();
	else updateOverMode();

}

void gameCore::updateIdleMode() {

	updateDepthStream();
	checkActiveBlocks();

}
void gameCore::updateRecoMode() {
...
void gameCore::updateDepthStream() {
	...
	streamContext.update();
	streamDepth.update();

	/* getting depth pixels directly from depth generator */

	depthRangeMask.setFromPixels(streamDepth.getDepthPixels(nearThreshold, farThreshold), 
								 streamDepth.getWidth(), // width = 640 px
								 streamDepth.getHeight(), // height = 480 px
								 OF_IMAGE_GRAYSCALE);

	depthStream.setFromPixels(depthRangeMask.getPixels(120, 0, 400, 480, tracking),
							  400,
							  480,
							  OF_IMAGE_GRAYSCALE);
...
void gameCore::updateGameScreen() {
	...
	if (!moveRight && moveLeft && isPossibleMove(pieceX - 1, pieceY)) pieceX--;
	if (moveRight && !moveLeft && isPossibleMove(pieceX + 1, pieceY)) pieceX++;
...
void gameCore::draw() {
	...
	if (isIdleMode) drawIdleMode();
	else if (isRecoMode) drawRecoMode();
	else if (isGameMode) drawGameMode();
	else drawOverMode();

}

void gameCore::drawIdleMode() {
...
void gameCore::switchMode(int mode) {
	...
	if (soundGame.isStreaming) soundGame.stop();
	if (soundIdle.isStreaming) soundIdle.stop();
	if (soundReco.isStreaming) soundReco.stop();
...
//--------------------------------------------------------------
														// LOGIC
//calculating how much a a pice will be worth
		...
		for (int j = 0; j < PIECEHEIGHT; j++) {
			...
			if (pieceBlocks[i][j]) {
				...
				switch (j) {
					...
					case 0: complexCalcR0++; break;
					
					case 1: complexCalcR1++; break;
					
					case 2: complexCalcR2++; break;
					
					case 3: complexCalcR3++; break;
...
int gameCore::addUpScore(int line) {
	...
	//cout<<tempScore<<" end tempscore"<<endl;
	return tempScore;
...
bool gameCore::isGameOver() {
	...
	for (int i = 0; i < GAMESCREENWIDTH; i++) {
		...
		if (gameBlocks[i][3] > 0) return true;
	...
	for (int i1 = pieceX, i2 = 0; i1 < pieceX + PIECEWIDTH; i1++, i2++) {
		for (int j1 = pieceY, j2 = 0; j1 < pieceY + PIECEHEIGHT; j1++, j2++) {
			...
			if (pieceBlocks[i2][j2]) {
				...
				//cout<<scoreOfStone<<" core of stones"<<endl;
			}
		}
	}
...
void gameCore::keyPressed(int key) {
	...
	switch (key) {
		...
		case '1': switchMode('idle'); break;

		case '2': switchMode('reco'); break;

		case '3': switchMode('game'); break;

		case '4': switchMode('over'); break;

		//case '5': deleteLine(18); break;

		/* increase volume of idle sound*/

		case '6':
			idleSoundVol += 0.1f;
			if (idleSoundVol > 1.0f) idleSoundVol = 1.0f;
			soundIdle.setVolume(idleSoundVol);
			break;
		...
		case '5':		
			idleSoundVol -= 0.1f;
			if (idleSoundVol < 0.0f) idleSoundVol = 0.0f;
			soundIdle.setVolume(idleSoundVol);
			break;
		...
		case '8':
			recoSoundVol += 0.1f;
			if (recoSoundVol > 1.0f) recoSoundVol = 1.0f;
			soundReco.setVolume(recoSoundVol);
			break;
		...
		case '7':		
			recoSoundVol -= 0.1f;
			if (recoSoundVol < 0.0f) recoSoundVol = 0.0f;
			soundReco.setVolume(recoSoundVol);
			break;
		...
		case '0':
			gameSoundVol += 0.1f;
			if (gameSoundVol > 1.0f) gameSoundVol = 1.0f;
			soundGame.setVolume(gameSoundVol);
			break;
		...
		case '9':		
			gameSoundVol -= 0.1f;
			if (gameSoundVol < 0.0f) gameSoundVol = 0.0f;
			soundGame.setVolume(gameSoundVol);
			break;
		...
		case '>': case '.':
			...
			if (farThreshold > streamDepth.getMaxDepth())
				farThreshold = streamDepth.getMaxDepth();
		...
		case '+': case '=':
			...
			if (nearThreshold > streamDepth.getMaxDepth())
				nearThreshold = streamDepth.getMaxDepth();
